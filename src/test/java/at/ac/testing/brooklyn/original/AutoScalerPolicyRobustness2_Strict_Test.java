 ...
 * 
 * We use this class as baseline for the comparison. Note that in this class we
 * ignore the timing-related parameters passed as input
 * 
 * @author alessiogambi
 * 
 */
public class AutoScalerPolicyRobustness2_Strict_Test {
	static {
		TimeExtras.init();
	}
	...
	public void testResizeUpStabilizationDelayIgnoresBlip(
		...
		// Original Values
		long _resizeUpStabilizationDelay = 1000L;
		long _minPeriodBetweenExecs = 0;
		...
		// to
		// 4
		Thread.sleep(_resizeUpStabilizationDelay - OVERHEAD_DURATION_MS);
	...
	public void testResizeUpStabilizationDelayTakesMaxSustainedDesired(
			...
			long resizeDownStabilizationDelay) throws Exception {
		// Original Values
		long _resizeUpStabilizationDelay = 1100L;
		long _minPeriodBetweenExecs = 0;
		//
		resizable.removePolicy(policy);
		policy = AutoScalerPolicy.builder()
		...
		Assert.assertTrue(timeToResizeTo2 >= _resizeUpStabilizationDelay
				- EARLY_RETURN_MS
				&& timeToResizeTo2 <= _resizeUpStabilizationDelay
						+ OVERHEAD_DURATION_MS, "Resizing to 2: time="
				+ timeToResizeTo2 + "; resizeUpStabilizationDelay="
				+ _resizeUpStabilizationDelay);
		...
		Assert.assertTrue(timeToResizeTo4 >= _resizeUpStabilizationDelay
				- EARLY_RETURN_MS
				&& timeToResizeTo4 <= _resizeUpStabilizationDelay
						+ OVERHEAD_DURATION_MS + _minPeriodBetweenExecs,
	...
	public void testResizeUpStabilizationDelayResizesAfterDelay(
		...
		// Original Values
		final long _resizeUpStabilizationDelay = 1000L;
		long _minPeriodBetweenExecs = 0;
		//
		resizable.removePolicy(policy);
		policy = AutoScalerPolicy.builder()
				...
				new Runnable() {
					public void run() {
						if (System.currentTimeMillis() - emitTime > (2 + emitCount
								.get()) * _resizeUpStabilizationDelay) {
		...
		Assert.assertTrue(
				resizeDelay >= (_resizeUpStabilizationDelay - EARLY_RETURN_MS),
	...
	public void testResizeDownStabilizationDelayIgnoresBlip(
		...
		// Original Values
		long _resizeDownStabilizationDelay = 1000L;
		long _minPeriodBetweenExecs = 0;
		//
		resizable.removePolicy(policy);
		...
		// to
		// 1
		Thread.sleep(_resizeDownStabilizationDelay - OVERHEAD_DURATION_MS);
		resizable.emit(AutoScalerPolicy.DEFAULT_POOL_OK_SENSOR,
	...
	public void testResizeDownStabilizationDelayTakesMinSustainedDesired(
			...
			long resizeDownStabilizationDelay) throws Exception {
		// Original Values
		long _resizeDownStabilizationDelay = 1100L;
		long _minPeriodBetweenExecs = 0;
		//
		resizable.removePolicy(policy);
		policy = AutoScalerPolicy.builder()
		...
		long timeToResizeTo2 = stopwatch.elapsed(TimeUnit.MILLISECONDS);
		Assert.assertTrue(timeToResizeTo2 >= _resizeDownStabilizationDelay
				- EARLY_RETURN_MS
				&& timeToResizeTo2 <= _resizeDownStabilizationDelay
						+ OVERHEAD_DURATION_MS, "Resizing to 2: time="
				+ timeToResizeTo2 + "; resizeDownStabilizationDelay="
				+ _resizeDownStabilizationDelay);
		...
		// well
		// So either we remove it from the parameters or we update the oracle !
		Assert.assertTrue(timeToResizeTo1 >= _resizeDownStabilizationDelay
				- EARLY_RETURN_MS
				&& timeToResizeTo1 <= _resizeDownStabilizationDelay
						+ OVERHEAD_DURATION_MS + _minPeriodBetweenExecs,
	...
	public void testResizeDownStabilizationDelayResizesAfterDelay(
		...
		// Original Values
		final long _resizeDownStabilizationDelay = 1000L;
		long _minPeriodBetweenExecs = 0;
		//
		resizable.removePolicy(policy);
		policy = AutoScalerPolicy.builder()
		...
		executeUntilSucceeds(MutableMap.of("timeout", TIMEOUT_MS),
				new Runnable() {
					public void run() {
						if (System.currentTimeMillis() - emitTime > (2 + emitCount
								.get()) * _resizeDownStabilizationDelay) {
							// first one may not have been received, in a
							// registration
							// race
							resizable.emit(
		...
		long resizeDelay = System.currentTimeMillis() - emitTime;
		Assert.assertTrue(
				resizeDelay >= (_resizeDownStabilizationDelay - EARLY_RETURN_MS),
				"resizeDelay=" + resizeDelay);
	}
