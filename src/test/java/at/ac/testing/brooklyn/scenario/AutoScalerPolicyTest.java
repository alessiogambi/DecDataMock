...
public class AutoScalerPolicyTest extends
	...
	public void testShrinkColdPool() throws Exception {
		...
		ScenarioHandler scenarioHandler = new ScenarioHandler.Builder()
				.metricLowerBound(4 * 10).metricUpperBound(4 * 20)//
	...
	public void testGrowHotPool() throws Exception {
		...
		// Specify the general configurations
		ScenarioHandler scenarioHandler = new ScenarioHandler.Builder()
				.metricLowerBound(2 * 10).metricUpperBound(2 * 20)//
				.withTimeUnit(TimeUnit.MILLISECONDS).build();
		DataMockLoadGenerator mockedDataGeneration = scenarioHandler
				.at(0)
				.startWith(
	...
	// TODO First resize can be simply scaleDown()
	...
	public void testResizeUpStabilizationDelayTakesMaxSustainedDesired()
		...
		executeUntilSucceeds(MutableMap.of("period", 1, "timeout", TIMEOUT_MS),
				new Runnable() {
					public void run() {
						assertTrue(resizable.getCurrentSize() >= 2,
		...
		assertTrue(timeToResizeTo2 >= resizeUpStabilizationDelay
				- EARLY_RETURN_MS
		...
		assertTrue(timeToResizeTo4 >= resizeUpStabilizationDelay
				- EARLY_RETURN_MS
	...
	public void testResizeUpStabilizationDelayResizesAfterDelay() {
		...
		// final Map<String, Object> need4 = (new ThresholdBasedPolicy(1 * 10, 1
		// * 20)).resize(1, 4).getSensorReading();
	...
	public void testResizeDownStabilizationDelayTakesMinSustainedDesired()
		...
		long timeToResizeTo2 = stopwatch.elapsed(TimeUnit.MILLISECONDS);
		assertTrue(timeToResizeTo2 >= resizeDownStabilizationDelay
				- EARLY_RETURN_MS
		...
		assertTrue(timeToResizeTo1 >= resizeDownStabilizationDelay
				- EARLY_RETURN_MS
